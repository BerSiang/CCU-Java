 ...
 * [A]102
 * [TA's advise]
 * 1.nextDouble()這個method讓你的程式更為謹慎, 可以讓使用者輸入到正確是一個很不錯的作法, 但x-axis跟y-axis是否可以出現負值呢? 另外你寫的正規表示式似乎不允許你輸入浮點數
 * 試試看用這個正規表示式允許輸入小數點[0-9]+\\.[0-9]+, 這作法很有趣, extra bonus +2.
 * 2.getArea(),getPerimeter()這兩個method本身結果就已經是double了, 無須額外的強制轉型.
 * 3.或許等確保輸入值正確後再產生物件可以讓你的做法更簡單也減少function call的cost, 但你的作法是相當謹慎的, 看的出來你C#學的很不錯. 
 * */
...
class RectangleTester {
	public static void main(String[] args) {
		...
		System.out.print("\nObject name:rect1\n" + rect1.toString() + "\nArea = " + rect1.getArea() + "\nPerimeter = "
				+ rect1.getPerimeter() + "\n\n");
		System.out.println(
				"Object name:rect2\n" + rect2.toString() + "\nArea = " + (double) rect2.getWidth() * rect2.getHeight()
		...
		/* 顯示 rect3 物件詳細資訊 */
		System.out.printf("%nObject name:rect3%n%s%nArea = %f%nPerimeter = %f%n", rect3.toString(), rect3.getArea(),
				rect3.getPerimeter());
	}/* 主程式結束 */

	/*
	 * 由於使用者輸入資料的不可預測性 建立nextDouble方法來偵測及糾正輸入資料使其符合預期 以避免程式不可預期的例外狀況
	 */

	/**
	 * 建立靜態的Scanner物件以接受使者輸入資料 在本類別載入時及建立
	 */
	static Scanner con = new Scanner(System.in);

	/**
	 * 建立靜態的 nextDouble 方法 負責偵測及糾正使用者輸入資料是否符合預期 在本類別載入時及建立
	 * 
	 * @param a
	 *            預期的輸入資料類型 (a = 0 時接受任何數字, a > 0 時僅接受正數, a < 0 時僅接受負數)
	 */
